...
/* ═══════════════════════════════════════════════════════════
   EC@RT — Zone 05 BAD  · version tablette allégée
   16 fragments (1/image) · proximity throttlée · drag + reset
   ═══════════════════════════════════════════════════════════ */
...
const CFG = {
  ...
  fragmentsPerImg:  1,        // ↓ était 3 — 16 fragments au lieu de 48
  minSize:          100,
  maxSize:          220,
  proximityDist:    110,
  energyDecay:      0.968,
  instabilityDecay: 0.980,
  driftStrength:    0.022,    // ↓ dérive réduite
  breathStrength:   0.004,    // ↓ souffle réduit
  maxLines:         6,        // ↓ était 14
  saveDelay:        200,
  minZoom:          0.5,
  maxZoom:          3.5
...
/* ── Contour irrégulier (12 pts — compatible CSS existant) ── */
function jaggedPolygon() {
  return {
    '--p1x': `${rand(0,6).toFixed(1)}%`,    '--p1y': `${rand(3,18).toFixed(1)}%`,
    '--p2x': `${rand(12,22).toFixed(1)}%`,  '--p2y': `${rand(-8,8).toFixed(1)}%`,
    '--p3x': `${rand(30,44).toFixed(1)}%`,  '--p3y': `${rand(0,13).toFixed(1)}%`,
    '--p4x': `${rand(58,72).toFixed(1)}%`,  '--p4y': `${rand(-8,10).toFixed(1)}%`,
    '--p5x': `${rand(88,100).toFixed(1)}%`, '--p5y': `${rand(8,25).toFixed(1)}%`,
    '--p6x': `${rand(94,104).toFixed(1)}%`, '--p6y': `${rand(35,52).toFixed(1)}%`,
    '--p7x': `${rand(86,100).toFixed(1)}%`, '--p7y': `${rand(72,96).toFixed(1)}%`,
    '--p8x': `${rand(64,78).toFixed(1)}%`,  '--p8y': `${rand(88,106).toFixed(1)}%`,
    '--p9x': `${rand(42,54).toFixed(1)}%`,  '--p9y': `${rand(92,104).toFixed(1)}%`,
    '--p10x':`${rand(18,32).toFixed(1)}%`,  '--p10y':`${rand(86,102).toFixed(1)}%`,
    '--p11x':`${rand(-4,10).toFixed(1)}%`,  '--p11y':`${rand(62,82).toFixed(1)}%`,
    '--p12x':`${rand(-6,9).toFixed(1)}%`,   '--p12y':`${rand(28,48).toFixed(1)}%`
  };
}
...
class BadFragment {
  ...
  _build() {
    ...
    el.style.willChange = 'transform'; // ← hint GPU

    el.appendChild(Object.assign(document.createElement('div'), { className: 'bad-fragment__halo' }));
    el.appendChild(Object.assign(document.createElement('div'), { className: 'bad-fragment__paper' }));
    ...
    const vars = {
      '--w': `${this.w}px`, '--h': `${this.h}px`,
      '--img': `url("${this.src}")`,
      '--bgw': `${bgW}px`, '--bgh': `${bgH}px`,
  ...
  _bind() {
    ...
    this.el.addEventListener('pointermove', e => {
      ...
      const p = screenToSurface(e.clientX, e.clientY);
      this.x = clamp(p.x - this.offsetX, -this.w * 0.35, surfaceW - this.w * 0.65);
      this.y = clamp(p.y - this.offsetY, -this.h * 0.35, surfaceH - this.h * 0.65);
  ...
  update(now) {
    ...
    const opa    = clamp(0.84 + breath * 0.022 + this.proximity * 0.07, 0.68, 0.96);
    const halo   = clamp(this.energy * 0.50 + this.proximity * 0.20, 0, 0.55);

    /* Chaîne de transform en une seule affectation */
...
function updateProximity() {
  fragments.forEach(f => { f.proximity *= 0.80; });
  ...
  for (let i = 0; i < fragments.length; i++) {
    ...
    for (let j = i + 1; j < fragments.length; j++) {
      ...
      if (d2 < dist2 && li < CFG.maxLines) {
        ...
        ln.setAttribute('stroke', `rgba(255,175,90,${(force * 0.22).toFixed(2)})`);
        ln.setAttribute('stroke-width', (0.25 + force * 0.60).toFixed(2));
        ln.style.display = '';
      }
    }
  }

  for (let i = li; i < linePool.length; i++) linePool[i].style.display = 'none';
}
...
function tick(now) {
  /* Proximity throttlée — 1 frame sur 2 → charge O(n²) divisée */
  if (frameCount % 2 === 0) updateProximity();
...
const UI_SELECTORS = [
  '.sidebar', '#badReset', '#soundToggle', '#infoToggle',
  '#mobileMapToggle', '#soundPanel', '#infoPanel', '#mobileMapOverlay'
];
...
function bindNavigation() {
  ...
  /* Pan — pointeur sur la surface vide */
  surface.addEventListener('pointerdown', e => {
    if (e.target.closest('.bad-fragment') || e.target.closest('button') || e.target.closest('a')) return;
    e.preventDefault();
    panning   = true;
  ...
  surface.addEventListener('touchend',   () => { pinchDist = null; });
  surface.addEventListener('touchcancel',() => { pinchDist = null; });
...
function init() {
  ...
  if (resetButton) resetButton.addEventListener('click', clearState);
